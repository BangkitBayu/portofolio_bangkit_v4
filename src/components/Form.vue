...
<template>
  <div>
    <form action="https://formspree.io/f/xpwvkzaz" method="POST"
      class="border border-third rounded-lg p-4 mt-4 flex flex-col space-y-3 w-full" autocomplete="on">
      ...
      <button v-on:submit="sendMessage" :disabled="v$.$invalid" class=" w-full p-2 bg-brand rounded-sm text-primary">Send
        Message</button>
    </form>
  </div>
</template>
